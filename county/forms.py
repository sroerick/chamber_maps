    ...
    def save(self):
        ...
        for line in records:
            ...
            if self.cleaned_data["form_select"] == 'countyMap':
                ...
                try:
                    map_object = countyMap.objects.get(
                        name__iexact=(polygon_name))
                except(countyMap.DoesNotExist):
                    try:
                        map_object = countyMap.objects.annotate(similarity=TrigramSimilarity(
                            'name', polygon_name), ).filter(similarity__gt=0.3).order_by('similarity')[0]
                    except():
                        pass
                ...
                input_data.mapname = mapMetaData.objects.get(
                    mapname=self.cleaned_data["text"])
                input_data.save()
            elif self.cleaned_data["form_select"] == 'stateTiger':
                input_data = mapGeometry()
                input_data.countyname = line[0]
                input_data.fips = stateTiger.objects.get(
                    name=(line[0].strip())).statefp
                input_data.floatdata = line[1].replace(",", "")
                try:
                    input_data.description = line[2]
                except(IndexError):
                    input_data.description = " "
                input_data.geometry = stateTiger.objects.get(
                    name=(line[0])).geom
                input_data.mapname = mapMetaData.objects.get(
                    mapname=self.cleaned_data["text"])
                input_data.save()
